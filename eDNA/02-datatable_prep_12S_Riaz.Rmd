...
```{r}
## read in edited df 
taxonomic_choice <- read_xlsx("data/BLASToutput/SampleReport_taxonomic_ID_edited.xlsx") %>%
    ## selecting only columns needed 
...
ASV_table_taxID_edited %>% rowwise() %>%
  mutate(across(.cols = (7:ncol(.)),            
                .fns = ~ ifelse((.x/ASV_sum)>0.001, NA, .x))) %>% ungroup() %>% write_xlsx("data/ASV_reads_filtered_out.xlsx")
...
ASV_table_taxID_filtered %>% dplyr::select(ASV_ID, Species_name, Common_name, Category, ASV_sum, ASV_rank) %>%
  write_xlsx("data/ASV_breakdown.xlsx")
```
